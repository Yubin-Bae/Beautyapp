...
def analyze_personal_color():
    ...
    try:
        ...
        tone = result['tone']  # 분석된 톤
        metrics = result['metrics']  # Lab_b와 hsv_s 값 포함한 metrics

        print(f"분석 완료: {tone}")  # 결과를 콘솔에 출력
        return jsonify({'colorType': tone, 'metrics': metrics})  # 결과를 클라이언트로 JSON 형태로 반환
...
def upload_image():
    ...
    file = request.files['file']
    color = request.form.get('color', '#FFFFFF')  # 기본값으로 립스틱 색상 설정
    file_path = os.path.join("uploads", file.filename)
    file.save(file_path)
